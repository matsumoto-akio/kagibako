...
![起動画面。「このMacを調べる」を押すだけです](docs/screenshot.png)

調べ終わると、まず「いますぐ対処した方がいい」件数だけが出ます。

![結果画面。いますぐ対処した方がいいキーの件数と、やることが3つ](docs/screenshot-2.png)

そのあと、ファイルごとに「なぜ危ないか」と「何をすればいいか」が順番に出ます。

![手順画面。ファイルごとの対処手順とコマンド](docs/screenshot-3.png)
...
4. 消すのは `~/.zshrc` の中の「キーを書いた1行」です。
   `export ANTHROPIC_API_KEY=sk-ant-...` のように書かれている行を、まるごと削除します。

5. 消したら困った(ツールが動かなくなった)場合だけ、次に進んでください。
...
6. 最後に、キーを配っているサイトでキーを再発行(新しいキーを作り直すこと)します。
   **順番が大事です。**

   1. 新しいキーを発行する
   2. 使っている場所を全部、新しいキーに差し替える
   3. 最後に、古いキーを失効(使えなくすること)させる

   ※ 先に古いキーを止めると、動いているものが止まります。

#### ツールのグローバル設定(`~/.claude/settings.local.json` など)

こちらも同じで、**まず、そのファイルがどのツールのものか確認**してください。
公式のログインがあるツールなら、キーを消してログインし直すだけで済みます。
そうでないツールは、キーを消すと動かなくなります。
...
この場所は**「そのままでよい可能性が高い」**です。
プロジェクトのフォルダの中の `.env` にキーを書くのは通常の運用で、移したり消したりする必要はありません。
確認するのは1点だけ、`.gitignore`(GitHubに上げないファイルを指定する設定)に入っているかどうか。
